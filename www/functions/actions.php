...
function demute($username){
	$mysqli = get_link();
	$query = mysqli_prepare($mysqli, 'DELETE FROM mute WHERE BINARY username = ?');
	mysqli_stmt_bind_param($query, 's', $username);
	mysqli_stmt_execute($query);
}
function deban($username){
	$mysqli = get_link();
	$query = mysqli_prepare($mysqli, 'DELETE FROM ban WHERE BINARY username = ?');
	mysqli_stmt_bind_param($query, 's', $username);
	mysqli_stmt_execute($query);
}
function is_mute($username){
	$mysqli = get_link();
	$query = mysqli_prepare($mysqli, 'SELECT end FROM mute WHERE BINARY username = ?');
	mysqli_stmt_bind_param($query, 's', $username);
	mysqli_stmt_execute($query);
...
function is_banned($username){
	$mysqli = get_link();
	$query = mysqli_prepare($mysqli, 'SELECT msg FROM ban WHERE BINARY username = ?');
	mysqli_stmt_bind_param($query, 's', $username);
	mysqli_stmt_execute($query);
...
function set_mute($username, $min, $hour, $day){
	...
	$mysqli = get_link();
	$query = mysqli_prepare($mysqli, 'SELECT id FROM mute WHERE BINARY username = ?');
	mysqli_stmt_bind_param($query, 's', $username);
	mysqli_stmt_execute($query);
	$i = 0;
	...
	if($i == 0){
		$mysqli = get_link();
		$query = mysqli_prepare($mysqli, 'INSERT INTO mute (username, start, end) VALUES (?, NOW(), ?)');
		mysqli_stmt_bind_param($query, 'ss', $username, $end);
		mysqli_stmt_execute($query);
	
	}else{
		$mysqli = get_link();
		$query = mysqli_prepare($mysqli, 'UPDATE mute SET start = NOW(), end = ? WHERE BINARY username = ?');
...
function set_rank($username, $rank){
	...
	if(isset($ranks[$rank])){
		$mysqli = get_link();
		$query = mysqli_prepare($mysqli, 'UPDATE users SET rank = ? WHERE BINARY username = ?');
...
function set_ban($username, $reason){
	$mysqli = get_link();
	$query = mysqli_prepare($mysqli, 'INSERT INTO ban (username, msg) VALUES (?, ?)');
	mysqli_stmt_bind_param($query, 'ss', $username, $reason);
	mysqli_stmt_execute($query);
}
function check_rank($username, $rank){
	$mysqli = get_link();
	$query = mysqli_prepare($mysqli, 'SELECT id FROM users WHERE BINARY username = ? AND rank = ?');
...
function get_rank($username){
	$mysqli = get_link();
	$query = mysqli_prepare($mysqli, 'SELECT rank FROM users WHERE BINARY username = ?');
	mysqli_stmt_bind_param($query, 's', $username);
	mysqli_stmt_execute($query);
