...
function private_messages($user, $contact){
	...
	if($i > 0){
		...
		//die(var_dump(is_friend($_SESSION['username'], $contact)));
		if(is_friend($_SESSION['username'], $contact)){
		//if($i > 0){
			$query = mysqli_prepare($mysqli, 'SELECT sender, contact, message, date FROM private WHERE (BINARY sender = ? OR BINARY sender = ?) AND
				(BINARY contact = ? OR BINARY contact = ?) AND isTyping = 0 ORDER BY id DESC LIMIT 80');
			mysqli_stmt_bind_param($query, 'ssss', $_SESSION['username'], $contact, $_SESSION['username'], $contact);
			mysqli_stmt_execute($query);
...
function display_private_chat($sender, $contact){
	echo "<div class=\"page-header\">
		...
		<div class=\"messages\">";
	private_messages($_SESSION['username'], $contact);
	echo "</div>
...
function send_private_message($user, $contact, $message){
	...
	mysqli_stmt_bind_param($query, 'sss', $_SESSION['username'], $contact, $message);
	mysqli_stmt_execute($query);
}
