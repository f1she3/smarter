<?php
	...
	if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
		...
		require_once '../functions/get_private.php';
		if(is_logged()){
			...
			if($i > 0){
				$one = 1;	
				$req = mysqli_prepare($mysqli, 'SELECT sender, date FROM private wHERE contact = ? AND is_typing = ?');
				mysqli_stmt_bind_param($req, 'si', $_SESSION['name'], $one);
				...
				$query = mysqli_prepare($mysqli, 'SELECT sender, contact, message, date FROM private WHERE (BINARY sender = ? OR
					BINARY sender = ?) AND (BINARY contact = ? OR BINARY contact = ?) AND is_typing = 0 ORDER BY id DESC LIMIT 80');
				mysqli_stmt_bind_param($query, 'ssss', $_SESSION['name'], $contact, $_SESSION['name'], $contact);
				mysqli_stmt_execute($query);
				...
				if($i > 0){
					...
					mysqli_stmt_bind_param($query, 'ss', $contact, $_SESSION['name']);
					mysqli_stmt_execute($query);
